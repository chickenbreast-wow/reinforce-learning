...
UPDATE_GLOBAL_ITER = 10  # 글로벌 업데이트 주기
GAMMA = 0.99
LR = 0.0001
EQUILIBRIUM_SCORE = 400  # 목표 점수
NUM_WORKERS = 8  

# ----------------------------------------
# 경로 설정 (현재 실행 파일 위치 기준)
# ----------------------------------------
try:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
except NameError:
    BASE_DIR = os.getcwd()  # 대화형 환경 등 __file__이 없을 경우

EXCEL_FILENAME = os.path.join(BASE_DIR, "training_results.xlsx")
GIF_FILENAME = os.path.join(BASE_DIR, "cartpole_demo.gif")
...
if __name__ == "__main__":
    ...
    print(f"System: Starting training with {NUM_WORKERS} workers...")
    ...
    while True:
        ...
        if all(not w.is_alive() for w in workers):
            # 워커가 다 죽었는데 큐에 남은게 있을 수 있으므로 한 번 더 체크
            try:
                while True:
                    record = data_queue.get_nowait()
                    data_list.append(record)
